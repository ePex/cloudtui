...
Plan: [2026-07-14_feat-tui-theme-refresh-plan.md](2026-07-14_feat-tui-theme-refresh-plan.md)
...
1. **`tui/internal/config/config.go`** — `Palette` gains `Background`,
...
2. **`tui/internal/config/config_test.go`** — update `TestDefault` and
   `TestLoadFullOverride` to the new field set/values.
   Status: done.

3. **`tui/config.example.yaml`** — document every new/changed field under
   `colors:`, matching `Default()`.
   Status: done.

4. **`tui/internal/app/theme.go`** (new) — `applyTheme(p config.Palette)`
   (sets `tview.Styles.*`) and `styleList(l *tview.List, p config.Palette)
   *tview.List` (selection colors).
   Status: done.

5. **`tui/internal/app/theme_test.go`** (new) — `applyTheme` followed by
   `tview.NewBox()` has `GetBorderColor()`/`GetBackgroundColor()` matching
   the palette passed in.
   Status: done.

6. **`tui/internal/app/app.go`** — call `applyTheme(cfg.Colors)` right
...
7. **`tui/internal/app/topbar.go`** — one-column divider between the info
...
8. **`tui/internal/app/topbar_test.go`** — update
...
12. **`tui/internal/app/queues_test.go`** — update the 3 references to the
    removed `statusReadyText` constant to `a.readyText()`.
    Status: done.

13. **`tui/internal/app/settings.go`** — wrap the settings list and the
    AWS-profile picker list construction with `styleList`.
    Status: done.

14. **Verify** — `gofmt -l .`, `go vet ./...`, `go test ./...`,
