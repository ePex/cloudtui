...
Plan: [2026-07-10_feat-aws-profile-selection-plan.md](2026-07-10_feat-aws-profile-selection-plan.md)

Each task below needs explicit manual approval before it is implemented.

1. **`tui/internal/awsprofile/awsprofile.go` + `awsprofile_test.go`** —
...
5. **`tui/internal/app/settings.go` + `settings_test.go`** —
...
6. **`tui/internal/app/app.go` + `app_test.go`** — wire
...
7. **`tui/config.example.yaml`** — document `aws.profile` (normally set
   via the picker, not hand-edited).
   Status: done.

8. **Verify** — `gofmt -l .`, `go vet ./...`, `go test ./...` clean/
