...
- Real `tui` against `local-fibu-proxy` (reference API): re-run bugfix
...
- `curl` against the isolated instance: `filter.maxCount=1` and
  `filter.jmsType=<nonexistent>` on `esprit.businesspartner.fiproxy.fibu`
...
- Real `tui` against `local-fibu-proxy` (the actual reference API):
