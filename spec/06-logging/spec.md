...
- Logger initialization lives in `cmd/cloudtui/main.go`, before `app.New()` is called (so applying the theme and building the shell are themselves covered by the startup log entries).
