...
4. [x] **`tui/internal/app/help.go` + `help_test.go`** — `newHelpModal(cfg)`
   (bordered keybinding list) and `centered(p, width, height)` (modal
   centering helper).
   Status: done.

5. [x] **`tui/internal/app/app.go`** — `App` gains `rootPages`,
...
6. [x] **`tui/internal/app/app_test.go`** — update existing tests for the
...
7. [x] **Verify** — `gofmt -l .`, `go vet ./...`, `go test ./...` clean/
