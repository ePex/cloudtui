...
- one VHS tape per scenario (`docs/demo/<scenario>.tape`) drives the real
  TUI and writes `docs/demo/<scenario>.gif`. Needs VHS 0.11.0 (0.12.0
  writes no GIF). VHS drives a headless browser, so it doesn't run in a
  sandboxed shell.
- `docs/demo/README.md` explains regenerating the GIFs and what to check
  ...
  - `:theme <name>`, typed while hidden, rather than the picker, so the
    bundled theme list isn't shown
