...
Plan: [2026-07-10_feat-richer-color-palette-plan.md](2026-07-10_feat-richer-color-palette-plan.md)

Each task below needs explicit manual approval before it is implemented.

1. **`tui/internal/config/config.go`** — `Palette` gains `Success`/
...
2. **`tui/internal/config/config_test.go`** — update `TestDefault`'s
...
3. **`tui/config.example.yaml`** — document `success`/`warning`/`error`
   and the `views:` map.
   Status: done.

4. **`tui/internal/app/app.go`** — add the `bordered` interface
...
5. **`tui/internal/app/app_test.go`** — assert a default-mapped view
...
6. **Verify** — `gofmt -l .`, `go vet ./...`, `go test ./...` clean/
