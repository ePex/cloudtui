...
- **`internal/view`** — resource screens (queues, messages, SSM params, logs, ...). Depend on `ui.Host` or one of `ui`'s narrower per-resource host interfaces (`SSMParamsHost`, `SecretsHost`, `CloudWatchLogsHost`, `DatadogLogsHost`, `CodePipelineHost`, `MessagesHost`, `MessageDetailHost` — each embeds `ui.Host`), never on a concrete `*App`.
...
**`internal/dialog`** — the ~16 modal overlay types, one file each: `confirm.go` (`ConfirmDialog`), `movepicker.go` (`MovePicker`), `sendmessage.go` (`SendMessageOverlay`), `snippetpicker.go` (`SnippetPicker`), `snippetbrowser.go` (`SnippetBrowser`: the snippet folder list, shared by the picker and the Snippets view), `snippetsave.go` (`SnippetSaveDialog`), `snippeteditor.go` (`SnippetEditor`), `textprompt.go` (`TextPrompt`: a reusable one-field prompt), `connections.go` (`ConnManager` + `ConnEditor` — two types in one file, since they're each other's sibling), `messagefilter.go` (`MessageFilter`), `timerangemodal.go` (`TimeRangeModal`), `datadogsettings.go` (`DatadogEditor`), `amqmanagersettings.go` (`AMQManagerSettingsEditor`: Settings → AMQ Manager, spec/07), `themepicker.go` (`ThemePicker`), `awsprofiles.go` (`AWSProfilesPicker`). Plus `hosttest_test.go`/`dialogtest_test.go` — a shared `ui.Host` test double used across the dialog package's tests.

**`internal/view`** — the resource views and their detail-view companions, one file each: `queues.go`, `messages.go`, `message_detail.go`, `ssmparams.go`, `paramdetail.go`, `secrets.go`, `secretdetail.go`, `logs.go` (CloudWatch log-group list), `logsearch.go` (CloudWatch log search), `logdetail.go`, `datadoglogs.go`, `datadoglogdetail.go`, `codepipelinelist.go`, `codepipelinedetail.go`, `settings.go`, `snippets.go` (`SnippetsView`, the snippet library, which takes the confirm dialog, `SnippetEditor` and `TextPrompt`), `log.go` (the app's own debug-log viewer — distinct from `logs.go`), `pipelinewatcher.go` (`PipelineWatcher` — the CodePipeline background poller, headless, no `ui.View`), `wraptext.go` (`dynamicWrapWidth` — shared free-text-column wrapping for tables, sized from the table's actual rendered width).
...
The contract every dialog depends on:
...
type MessageDetailHost interface {
    Host
    CopyToClipboard(data string) // `c` copies the message (spec/08)
}
```
...
- `MessageDetailHost` — `message_detail.go`
- Plain `ui.Host` — `queues.go`, `settings.go`, `snippets.go` call no resource-specific method at all (`log.go` takes no host at all)
...
`*App` proves it satisfies every one of the 8 interfaces via 7
compile-time assertions in `internal/app/viewhost.go`
(`SSMParamsHost` through `MessageDetailHost` — `Host` has its own assertion
...
- `SendMessageOverlay` takes `SnippetPicker` (its **Load snippet…** button) and `ConfirmDialog` (its "replace fields?" confirmation) as constructor parameters (`NewSendMessageOverlay(host, snippetPicker, confirm)`). `SnippetSaveDialog` and `SnippetEditor` take `ConfirmDialog` for their overwrite (and, for the editor, discard-changes) confirmations (`NewSnippetSaveDialog(host, store, confirm)`, `NewSnippetEditor(host, store, confirm)`). `SnippetPicker` is built around a `SnippetBrowser` (composition, not a sibling reference). All snippet dialogs, and the Snippets view, share one `*snippet.Store` (spec/22).
- A confirmation raised on top of another overlay that stays open uses `ConfirmDialog.ShowWithCancel(question, onConfirm, onCancel)`. "No"/Esc call `onCancel`, which gives focus back to that overlay, instead of `FocusMain`. Plain `Show` keeps the `FocusMain` behavior.
...
5. **The dialogs that views construct directly are built first** (`confirm`, `movePicker`, then the snippet store with `snippetPicker`, `snippetSave`, `textPrompt` and `snippetEditor`, then `sendMessage`, which needs `snippetPicker`, then `messageFilter`, `jmsTypePrompt`, `timeRangeModal`, `connManager`, `datadogEditor`, `amqManagerEditor`, `themePicker`, `awsProfiles`) — they must exist before the views that take them as constructor parameters. `connEditor` is the one exception: it's constructed *after* the dialog-coupled views below, right where it's wired into the overlay stack, because it needs `connManager` (already built) but nothing constructed after it depends on `connEditor` existing early.
...
- **Task-shaped Host methods over raw field mutation.** `Host.SaveConnection`/`DeleteConnection`/`SetActiveAWSProfile`/`SaveDatadogConfig`/`SaveAMQManagerSettings` exist so dialogs describe *what* they want done (in domain terms) rather than reaching into `a.cfg.Connections` themselves — this is what actually makes a dialog's `save()` method portable off `*App`, not just the interface boundary by itself.
...
- **Some dialogs keep a direct sibling reference instead of going through `Host`.** The inter-dialog dependencies in the whole package are:
  - `ConnManager` and `ConnEditor`, which reference each other
  - `ConnManager` → `ConfirmDialog`
  - `SendMessageOverlay` → `SnippetPicker` and `ConfirmDialog`
  - `SnippetSaveDialog` → `ConfirmDialog`
  - `SnippetEditor` → `ConfirmDialog`

  They're passed as constructor parameters rather than added to `Host`, since `Host` is meant to expose only shell-level capabilities, not let one dialog type discover another by name.
...
- **`clearTableBody`/`showStatusCell` (`internal/view/statuscell.go`) are the shared shape behind the same 5 views' `showError`/`showStatus`**, plus every other `for table.GetRowCount() > 1 { table.RemoveRow(...) }` clear-loop in those views' `repaint()` methods (and `CodePipelineDetailView`'s `Open()`/`Render()`). `clearTableBody(table)` is just the clear-loop, reused everywhere that shape appears, not only inside status rendering. `showStatusCell(table, col, text, color, title, clearState)` builds on it for the full "null cached state → clear body → write one cell → set title" shape; `clearState` is a closure because each view nulls different fields of different types (`[]awsssm.Parameter`, `[]awscodepipeline.StageStatus`, ...) — `statuscell.go` has no reason to know any specific view's struct shape. The column argument varies per view (1 for SSM/Secrets/CloudWatch Logs, which have a leftmost favorite-star column; 0 for the two CodePipeline views, which don't) and the title argument covers both the static-per-view-name case and `CodePipelineDetailView`'s per-pipeline dynamic title (`fmt.Sprintf(" %s ", dv.pipelineName)`) — both are just strings computed by the caller, not something the helper needs to special-case. **`QueuesView` deliberately isn't among the callers**, for the same reason it's absent from `runAWSLoad`'s callers above, plus its own `showError`/`showStatus` differ enough (no `SetTitle` call, no cached-state nulling, a different `SetExpansion` value) that it wouldn't be a clean fit without adding parameters only one caller would use.
- **A test that triggers async work (`search()`, `discoverFacetValuesFor()`, ...) through a tview widget callback rather than calling it directly must block the fake's fetch func and wait on `drawSignalingHost` before *and* after — not just after.** Found the hard way in `datadoglogs_test.go`: `tview.DropDown.SetCurrentOption` invokes its `SetSelectedFunc` callback *synchronously*, so a test that calls `SetCurrentOption` on a dropdown wired (via `refreshFilterDropdowns`) with the real `onSelect` — which calls `search()` — triggers a `go func(){...}()` *while `SetCurrentOption`'s own call is still on the stack*. That goroutine can reach `QueueUpdateDraw`/`SetOptions()` on the very same dropdown before `SetCurrentOption` finishes its own internal bookkeeping, racing it directly — and adding a `<-host.drawn` wait *after* the `SetCurrentOption` call doesn't help, because the race window is *inside* the call, not after it. Real tview can never hit this: `QueueUpdateDraw` always defers to a later point on the single event-loop goroutine, which by construction can't overlap with an input handler (like the one driving `SetCurrentOption`) still executing on that same goroutine. The fix — set the host's fetch func field (e.g. `searchDatadogLogsFn`) to block on an `unblock` channel *before* making the triggering call, so the spawned goroutine can't touch any widget state until the test explicitly releases it well after the triggering call (and any other synchronous-only assertions) have completed, then `close(unblock)` + `<-host.drawn` at the end so the goroutine can't leak past the test either. This is the general shape any future test needs if it drives async work through a widget callback instead of calling the `handleXResult`-style split-out method directly (the latter, used by most tests in this package, has no such hazard — it's synchronous by construction).
