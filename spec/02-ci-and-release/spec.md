...
- Cutting a release tag is a manual, explicit action (pushing a tag is a public, hard-to-reverse action) — never automated by CI itself. **Two tags are pushed together**: `vX.Y.Z` (triggers this workflow) and `tui/vX.Y.Z` (same commit, no workflow attached) — required because `tui/go.mod`'s module (`github.com/ePex/cloudtui/tui`) lives in a subdirectory, and Go's module versioning only resolves real tagged versions for `go install`/`go get` when the tag is prefixed with that subdirectory; a bare `vX.Y.Z` is invisible to Go's module resolution for this module (confirmed live: `go install .../cmd/cloudtui@latest` silently fell back to an arbitrary stale pseudo-version, and `@vX.Y.Z` hard-failed, until `tui/vX.Y.Z` existed). After pushing a brand-new `tui/vX.Y.Z` tag, expect a short propagation delay before `sum.golang.org`/`proxy.golang.org` index it — a fresh tag can 404 from the public checksum database for a few minutes; this is normal and needs no action.
...
- The Homebrew cask carries a `hooks.post.install` postflight that runs `xattr -dr com.apple.quarantine` on the installed binary. Without it, macOS Gatekeeper kills the binary on every launch (confirmed live: `spctl -a -vv` reports `rejected`) — Homebrew Cask quarantines any downloaded file, and this binary is only adhoc-signed (Go's default; no Apple Developer ID, no notarization, which would need a paid Apple Developer Program account not pursued for this project). This is GoReleaser's own documented pattern for a plain, unsigned-binary cask, not a novel workaround.
