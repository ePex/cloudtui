...
1. [x] **Annotate both existing specs** — append a one-line "Predates workflow
...
2. [x] **Retroactive plan + tasks for the Taskfile change** — write
...
3. [x] **Retroactive plan + tasks for the tui scaffold change** — write
...
4. [x] **`tui/internal/ui/views/views_test.go`** — table-driven test that
...
5. [x] **`tui/internal/app/app_test.go`** — `New()` registers all three views
...
6. [x] **Verify** — `gofmt -l .`, `go vet ./...`, `go test ./...` (or
