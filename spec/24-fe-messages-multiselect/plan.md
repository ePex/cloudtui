...
| `d` | `deleteMarked` | No marks → status hint, no dialog. Else confirm once, then `RemoveMessage` per marked ID in a goroutine, reload. |
| `m` | `moveMarked` | No marks → status hint, no picker. Else `showMovePicker` once, then `MoveMessage` per marked ID to the chosen target in a goroutine, reload. |
...
— used by both bulk operations so batches process in a predictable,
visually-matching order.
