...
func TestLogsViewShowStatusRendersMessage(t *testing.T) {
	...
	a.logsV.showStatus("AWS SSO session expired — opening browser to log in...")

	if got := a.logsV.table.GetCell(1, 0).Text; !strings.Contains(got, "opening browser") {
		t.Errorf("status cell = %q, want it to contain the status message", got)
	}
	fg, _, _ := a.logsV.table.GetCell(1, 0).Style.Decompose()
