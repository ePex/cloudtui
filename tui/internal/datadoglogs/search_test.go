...
func newTestLogEvent(ts time.Time) searchResponse {
	...
	resp.Data[0].Attributes.Service = "bar-proxy"
...
func TestBuildLogEventsPopulatesFields(t *testing.T) {
	...
	if e.Service != "bar-proxy" {
		t.Errorf("Service = %q, want %q", e.Service, "bar-proxy")
	}
	if e.Host != "host-1" {
...
func TestSearchSendsExpectedRequestAndParsesResponse(t *testing.T) {
	...
	got, hasMore, err := search(context.Background(), srv.URL, "tok-123", "env:testt service:bar-proxy", from, to)
	if err != nil {
	...
	if gotBody.Filter.Query != "env:testt service:bar-proxy" {
		t.Errorf("filter.query = %q, want %q", gotBody.Filter.Query, "env:testt service:bar-proxy")
	}
